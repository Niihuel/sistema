...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        console.log('[Dashboard] Loading dashboard summary...')
        const res = await fetch("/api/dashboard/summary", {
          credentials: "same-origin"
        })
        console.log('[Dashboard] Dashboard summary response:', res.status, res.ok)
        if (!res.ok) throw new Error("No autorizado o error de servidor")

        const text = await res.text()
        console.log('[Dashboard] Dashboard summary text:', text)

        if (text.trim()) {
          const jsonData = JSON.parse(text)
          console.log('[Dashboard] Dashboard summary data:', jsonData)
          setData(jsonData as typeof data)
        } else {
          console.warn('[Dashboard] Empty response from dashboard summary')
