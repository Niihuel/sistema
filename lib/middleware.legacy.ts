...
/**
 * Interface para contexto de autenticaci├│n mejorado con sistema Discord
 */
...
/**
 * Clase para errores de autenticaci├│n
 */
...
/**
 * FUNCI├ôN PRINCIPAL: requireAuth - Extraer y verificar autenticaci├│n
 * Corrige problemas de "Cannot read properties of undefined"
 */
export function requireAuth(req: NextRequest): AuthContext {
  try {
    // CORRECCI├ôN: Extraer token de forma segura
    let token: string | null = null;
    
    try {
      // 1. Intentar desde cookies (m├ís seguro)
    ...
    if (!token) {
      throw new AuthError('Token de autenticaci├│n requerido', 'TOKEN_REQUIRED', 401);
    }

    // CORRECCI├ôN: Verificar token JWT con manejo robusto de errores
    ...
    // CORRECCI├ôN: Validar estructura del payload para evitar undefined
    if (!payload.userId || typeof payload.userId !== 'number') {
      throw new AuthError('ID de usuario inv├ílido en token', 'INVALID_USER_ID', 401);
    }

    if (!payload.username || typeof payload.username !== 'string') {
      throw new AuthError('Nombre de usuario inv├ílido en token', 'INVALID_USERNAME', 401);
    }

    if (!payload.role || typeof payload.role !== 'string') {
      throw new AuthError('Rol de usuario inv├ílido en token', 'INVALID_ROLE', 401);
    }

    // Crear contexto de autenticaci├│n v├ílido
    ...
    throw new AuthError('Error de autenticaci├│n', 'AUTH_ERROR', 401);
  }
}

/**
 * FUNCI├ôN: requireRole - Verificar roles espec├¡ficos
 * Corrige problemas con verificaci├│n de roles undefined
 */
export function requireRole(context: AuthContext, allowedRoles: string[]): void {
  try {
    // CORRECCI├ôN: Validar contexto de entrada
    if (!context || typeof context !== 'object') {
      throw new AuthError('Contexto de autenticaci├│n inv├ílido', 'INVALID_CONTEXT', 401);
    }

    if (!context.role || typeof context.role !== 'string') {
      throw new AuthError('Rol no v├ílido en contexto', 'INVALID_ROLE', 401);
    }

    // CORRECCI├ôN: Validar roles permitidos
    ...
    if (normalizedAllowedRoles.length === 0) {
      throw new AuthError('No hay roles v├ílidos especificados', 'NO_VALID_ROLES', 500);
    }

    // CORRECCI├ôN: Verificar si el usuario tiene uno de los roles permitidos
    ...
    throw new AuthError('Error de verificaci├│n de roles', 'ROLE_ERROR', 500);
  }
}

/**
 * FUNCI├ôN: requireAdmin - Verificar permisos de administrador
 */
...
/**
 * FUNCI├ôN: requireTechnician - Verificar permisos t├®cnicos o superiores
 */
...
export function withAuth(handler: (req: NextRequest, context: AuthContext) => Promise<NextResponse>) {
  return async (req: NextRequest): Promise<NextResponse> => {
    try {
      ...
      return NextResponse.json(
        { error: 'Error de autenticaci├│n interno', code: 'INTERNAL_AUTH_ERROR' },
        { status: 500 }
      );
    }
  };
...
export function withAuthAndRole(
  ...
  return withAuth(async (req, context) => {
    try {
      ...
      return NextResponse.json(
        { error: 'Error de autorizaci├│n interno', code: 'INTERNAL_ROLE_ERROR' },
        { status: 500 }
      );
    }
  });
...
/**
 * FUNCI├ôN: requirePermission - Sistema de permisos granulares para producci├│n
...
export function requirePermission(context: AuthContext, permission: string): void {
  ...
  export const legacyPermissionToRoles: Record<string, string[]> = {
  ...
  const requiredRoles = legacyPermissionToRoles[permission];
  if (!requiredRoles) {
    // Si no encuentra el permiso espec├¡fico, rechazar acceso por seguridad
...
/**
 * FUNCI├ôN HELPER: Obtener informaci├│n del usuario desde token
 */
...
/**
 * FUNCI├ôN HELPER: Verificar si un rol es v├ílido
 */
...
/**
 * FUNCI├ôN HELPER: Comparar niveles de roles
 */
...
/**
 * FUNCI├ôN HELPER: Log de eventos de seguridad
 */
